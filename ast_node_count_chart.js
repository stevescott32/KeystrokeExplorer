...
class AstNodeCountChart {
  constructor() {
    ...
    create(asts) {
      ...
      this.displayAstNodeCountChart(formatted)
    }

    updatePlaybar(value) {
      console.log('updating the ast node count chart with value', value)
      const chartWidth = document.getElementById('ast_node_count_chart').clientWidth;
  ...
  displayAstNodeCountChart(data) {
    ...
    const svg = d3.select("#ast_node_count_chart")
          .append("svg")
          .attr("id", "ast_node_count_chart_svg")
